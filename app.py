...
st.set_page_config(page_title="FORECAST BY RAHUL", layout="wide")

# === Load model ===
model = joblib.load('ev_adoption_forecasting_unique_model.pkl')
...
st.markdown("""
    <style>
        ...
        .stApp {
            background: linear-gradient(to right,#c2f0f7, #9fd8df, #7fbfcb);
        }
    </style>
...
st.markdown("""
    <div style='text-align: center; font-size: 36px; font-weight: bold; color: #FFFFFF; margin-top: 20px;'>
        Forecasts Future EV Adoption Demand for a County in Washington State
    </div>
""", unsafe_allow_html=True)
...
st.image("EV.png", use_container_width=True)
...
# === Load data and encode if missing ===
@st.cache_data
def load_data():
